...
RSS_FEED_URL = os.getenv("RSS_FEED_URL", "https://example.com/feed/")
...
def main():
    ...
    new_ids = []
    # iterate newest-first so we notify older-to-newer (reverse later)
    to_notify = []
    for entry in entries:
        # use id fallback to link+title if no id
    ...
    for entry_id, text in reversed(to_notify):
        try:
            send_telegram(text)
            print(f"Sent notification for: {entry_id}")
        except Exception as e:
            print("Failed to send telegram:", e)

    if new_ids:
        # append new ids to seen list and save
